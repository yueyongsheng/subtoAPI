...
func TestGatewayService_SelectAccountForModelWithPlatform_RoutedOpenAIGroup(t *testing.T) {
	...
	repo := &mockAccountRepoForPlatform{
		accounts: []Account{
			...
			{ID: 1, Platform: PlatformOpenAI, Priority: 1, Status: StatusActive, Schedulable: true, Credentials: map[string]any{
				"model_mapping": map[string]any{requestedModel: requestedModel},
			}},
			{ID: 2, Platform: PlatformOpenAI, Priority: 5, Status: StatusActive, Schedulable: true, Credentials: map[string]any{
				"model_mapping": map[string]any{requestedModel: requestedModel},
			}},
		},
		accountsByID: map[int64]*Account{},
