...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if reqStream && account != nil && account.Platform == PlatformOpenAI {
		if reasoningEffort != nil {
			firstOutputEffort = strings.TrimSpace(*reasoningEffort)
		}
	...
	for {
		...
		if headerGuard != nil && headerGuard.stopHeaderWait() {
			...
			return nil, s.newOpenAIFirstOutputTimeoutError(
				...
				startTime, reqModel, firstOutputEffort, firstOutputTimeout, "response_headers", nil,
			)
		}
		if err != nil {
...
// openAIStreamEventIsPreamble is the compatibility name used by the native
// Responses and Chat Completions streaming paths. Preamble frames carry
// lifecycle metadata and must not count as the first semantic model output.
...
func openAIStreamDataStartsSemanticTTFT(data, eventType string) bool {
	...
	case "response.completed", "response.done", "response.incomplete", "response.cancelled", "response.canceled":
		// A usage-only terminal event is protocol bookkeeping, not model output.
		// Count a terminal event for TTFT only when it carries visible content.
...
func openAIStreamErrorEventShouldFailover(payload []byte, message string) bool {
	...
	return strings.Contains(combined, "temporary") ||
		...
		strings.EqualFold(strings.TrimSpace(message), "failed")
}
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	firstOutputTimedOut := make(chan struct{})
	firstOutputTimer := (*time.Timer)(nil)
	if account != nil && account.Platform == PlatformOpenAI {
		if timeout := s.openAIFirstOutputTimeout(""); timeout > 0 {
			firstOutputTimer = time.AfterFunc(time.Until(startTime.Add(timeout)), func() {
				if !clientOutputStarted && !semanticOutputSeen {
					close(firstOutputTimedOut)
					_ = resp.Body.Close()
				}
			})
		}
	...
	// Keep one complete SSE frame buffered before exposing it. This synthesizes
	// a missing EOF blank-line boundary and drops truncated JSON data frames
	// before they can reach strict downstream Responses decoders.
	...
	select {
	case <-firstOutputTimedOut:
		return resultWithUsage(), s.newOpenAIFirstOutputTimeoutError(
			ctx, c, account, opsUpstreamProxyID(account), opsUpstreamProxyName(account),
			startTime, originalModel, "", s.openAIFirstOutputTimeout(""), "semantic_output", resp.Header,
		)
	default:
	}
