...
func (i *PluginPackageInstaller) Install(ctx context.Context, reader io.Reader, installedBy *int64) (*PluginInstallation, error) {
	...
	// Windows keeps the ZIP backing file locked while zip.ReadCloser is open,
	// so close it before atomically moving the uploaded artifact out of staging.
...
func (i *PluginPackageInstaller) extractArchive(ctx context.Context, archive *zip.Reader, manifest PluginManifest, target string) error {
	...
	for path, expectedHash := range manifest.Files {
		...
		// Re-apply the runtime mode after closing the handle. Windows ignores
		// permission bits passed to OpenFile on some filesystems, which made the
		// extracted runtime appear non-executable in host validation.
		if path == manifest.Runtimes[manifest.RuntimeKey()].Path {
			if err := os.Chmod(destination, mode); err != nil {
				return fmt.Errorf("设置插件运行文件权限 %s 失败: %w", path, err)
			}
		}
