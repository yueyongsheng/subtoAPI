...
// deriveQuotaCheckResult converts a quota snapshot into the monitor status used
// by the timeline and availability calculations.
func deriveQuotaCheckResult(snapshot *domain.MonitorQuotaSnapshot, model string, checkedAt time.Time) *CheckResult { //nolint:unused // exercised by quota status tests
...
// quotaDegradedHint returns a concise explanation for a degraded snapshot.
func quotaDegradedHint(snapshot *domain.MonitorQuotaSnapshot) string { //nolint:unused // exercised by quota status tests
