...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	if account.Platform != PlatformGrok {
		if scoped, changed, scopeErr := applyCodexAccountIdentityClientMetadataRaw(body, codexAccountIdentitySource(c, account), getAPIKeyIDFromContext(c)); scopeErr != nil {
			return nil, scopeErr
		} else if changed {
			body = scoped
		}
	...
	for {
		...
		if account.Platform == PlatformGrok {
			...
			if turn > 1 && resp.StatusCode == http.StatusTooManyRequests {
				if rawInput, present := openAIWSHTTPBridgeRawField(body, "input"); present {
					var fullInput []json.RawMessage
					if err := json.Unmarshal(rawInput, &fullInput); err == nil {
						// `body` has account-scoped identity fields; replay the raw
						// client envelope so a replacement account can scope it anew.
						if retryPayload, retrySafe, retryErr := buildOpenAIWSCurrentTurnRetryPayload(payload, fullInput, true, originalModel); retryErr == nil && retrySafe {
