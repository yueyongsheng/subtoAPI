...
func TestForwardAsChatCompletions_StreamBareCapacityErrorReturnsFailoverWithoutOutput(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstreamBody := strings.Join([]string{
		`data: {"type":"response.created","response":{"id":"resp_capacity","model":"gpt-5.5","status":"in_progress","output":[]}}`,
		"",
		`event: error`,
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_chat_bare_capacity"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
...
func TestForwardAsChatCompletions_BufferedBareCapacityErrorReturnsFailoverWithoutOutput(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_chat_buffered_capacity"}},
