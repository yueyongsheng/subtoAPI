...
func cloneOpenAIWSRawMessages(messages []json.RawMessage) []json.RawMessage {
	if messages == nil {
		return nil
	}
	out := make([]json.RawMessage, len(messages))
	for i, message := range messages {
		out[i] = append(json.RawMessage(nil), message...)
	}
	return out
}
	...
	if !hasPreviousResponseID {
		...
		for _, item := range currentItems {
			...
			if isCodexToolCallOutputItemType(typ) {
				if id := strings.TrimSpace(gjson.GetBytes(item, "call_id").String()); id != "" {
					outputCallIDs[id] = struct{}{}
				}
			}
		}
		if hasContext || len(outputCallIDs) == 0 {
			return currentItems, currentExists
		}
		matching := make([]json.RawMessage, 0, len(previousFullInput)+len(currentItems))
		...
		matching = append(matching, currentItems...)
