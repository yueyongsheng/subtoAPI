...
func openaiCodexDisplayName(modelID string) string {
	...
	// GPT-6 Astra is exposed only under its exact public ID; the legacy
	// gpt-6 alias must keep a generic display name and never inherit Astra metadata.
	if normalized == "gpt-6" {
		return modelID
	}
...
func convertOpenAIModelListToCodexManifestForAccount(body []byte, account *Account) []byte {
	...
	for _, entry := range entries {
		...
		sanitizeAstraCodexToolCapabilities(capabilityModel, capabilities)
