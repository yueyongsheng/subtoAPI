...
func (s *OpenAIGatewayService) handleChatStreamingResponse(
	...
	processDataLine := func(payload string) bool {
		...
		// response.created/in_progress are Responses protocol preambles. They
		// must stay pending and must not become a visible Chat role chunk: doing
		// so commits the downstream response before a capacity/error event can
		// trigger account failover.
		var chunks []apicompat.ChatCompletionsChunk
		if !openAIStreamEventIsPreamble(event.Type) {
