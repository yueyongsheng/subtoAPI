...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	// Keep OpenAI preamble frames private until a semantic event commits the
	// attempt. This preserves failover when capacity errors follow metadata even
	// when the explicit deadline is disabled; successful streams still commit
	// the buffered preamble at their first terminal/output event.
	...
	// Validate complete SSE frames before staging or forwarding them. This also
	// synthesizes the missing blank-line boundary when an upstream closes after
	// a final line without the usual SSE terminator.
	...
	finalizeStream := func() (*openaiStreamingResult, error) {
		...
		if !sawTerminalEvent && !responsesSemanticOutputSeen && !clientDisconnected {
		...
		// Commit staged frames only after a terminal event. A preamble-only EOF
		// must remain invisible so the caller can fail over safely.
		if stageFirstOutput && firstOutputTimeout <= 0 && firstOutputStage != nil && firstOutputStage.Buffered() > 0 && !clientDisconnected {
			if err := firstOutputStage.CommitTo(w); err == nil {
				flusher.Flush()
				clientOutputStarted = true
			}
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			errorEventShouldFailover := false
			...
			if eventType == "response.failed" || eventType == "error" {
				...
				if !outputStarted {
					...
					if !cyberHit {
						if eventType == "error" {
							errorEventShouldFailover = openAIStreamErrorEventShouldFailover(dataBytes, failedMessage)
							shouldFailover = errorEventShouldFailover
						} else {
				...
				sawFailedEvent = true
				terminalFailurePending = eventType == "response.failed" || (eventType == "error" && errorEventShouldFailover)
			}
	...
	if streamInterval <= 0 && keepaliveInterval <= 0 && firstOutputTimeout <= 0 {
		...
		for documentScanner.Scan() {
			...
			// 必须继续读取，不适用提前结束。
			if sawTerminalEvent && terminalEventType != "error" && !eventInProgress && (!codexFailureTerminal || !sawBareError) {
	...
	for {
		...
		case <-keepaliveCh:
			...
			if eventInProgress || openAISSEScannerHasOpenFrame(documentScanner) {
				continue
			}
