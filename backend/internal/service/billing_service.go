...
// PublishedTokenPrices is the user-facing view of live billing prices. Values
// remain USD per token; handlers may scale them for display.
...
// GetPublishedModelPricing uses the same tier selection and model policy as
// live token billing, keeping both model-plaza endpoints on one price source.
...
func (s *BillingService) initFallbackPricing() {
	...
	for _, model := range yuexiangOpenAIChargedModels {
		pricing, _ := yuexiangOpenAIModelPricing(model)
		s.fallbackPrices[model] = pricing
	}
	// Kept for upstream compatibility, but not part of the charged-model allowlist.
