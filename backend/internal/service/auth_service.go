...
func (s *AuthService) createUserWithRegistrationEmailGuard(ctx context.Context, user *User) error {
	...
	if user.Balance <= 0 || s.redeemRepo == nil || s.entClient == nil || dbent.TxFromContext(ctx) != nil {
		return s.createUserWithRegistrationEmailGuardAndSignupBonus(ctx, user)
	}
	...
	txCtx := dbent.NewTxContext(ctx, tx)
	if err := s.createUserWithRegistrationEmailGuardAndSignupBonus(txCtx, user); err != nil {
		return err
	}
	return tx.Commit()
}

func (s *AuthService) createUserWithRegistrationEmailGuardAndSignupBonus(ctx context.Context, user *User) error {
	if err := s.createUserWithRegistrationEmailGuardOnly(ctx, user); err != nil {
		return err
	}
