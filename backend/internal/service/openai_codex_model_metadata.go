...
func sanitizeAstraCodexToolCapabilities(modelID string, capabilities map[string]json.RawMessage) {
	...
	delete(capabilities, "multi_agent_reasoning_effort")
	delete(capabilities, "multi_agent_version")
}

func sanitizeAstraCodexManifestFields(modelID string, fields map[string]json.RawMessage) bool {
	if !isOpenAIGPT6AstraModel(modelID) {
		return false
	}
	changed := false
	for _, field := range []string{"multi_agent_reasoning_effort", "multi_agent_version"} {
...
func accountCodexToolCapabilities(account *Account, modelID string) map[string]json.RawMessage {
	...
	sanitizeAstraCodexToolCapabilities(modelID, capabilities)
