...
func TestOpenAIGatewayServiceRecordUsage_OverdraftStillWritesUsageLog(t *testing.T) {
	newBalance := -0.25
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "resp_overdraft_recorded",
			Usage:     OpenAIUsage{InputTokens: 8, OutputTokens: 4},
			Model:     "gpt-5.1",
			Duration:  time.Second,
		},
		APIKey:  &APIKey{ID: 10051},
		User:    &User{ID: 20051, Balance: 0.01},
	...
	require.Equal(t, 1, usageRepo.calls)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, "resp_overdraft_recorded", usageRepo.lastLog.RequestID)
...
func TestOpenAIGatewayServiceRecordUsage_GPT56SeparatesCacheWriteForBillingAndStats(t *testing.T) {
	...
	svc.billingService = NewBillingService(svc.cfg, &PricingService{pricingData: map[string]*LiteLLMModelPricing{
		"gpt-5.6-sol": {
			InputCostPerToken:       17.5e-6,
			OutputCostPerToken:      105e-6,
			CacheReadInputTokenCost: 1.75e-6,
		},
	}})
	...
	require.InDelta(t, 700*17.5e-6, usageRepo.lastLog.InputCost, 1e-12)
	require.InDelta(t, 200*21.875e-6, usageRepo.lastLog.CacheCreationCost, 1e-12)
	require.InDelta(t, 100*1.75e-6, usageRepo.lastLog.CacheReadCost, 1e-12)
	require.InDelta(t, 50*105e-6, usageRepo.lastLog.OutputCost, 1e-12)
...
func TestOpenAIGatewayServiceRecordUsage_GroupAndAccountLongContextMustBothAllow(t *testing.T) {
	...
	baseInput := 300000 * 8.75e-6
	baseOutput := 2000 * 52.5e-6
...
func TestOpenAIGatewayServiceRecordUsage_GrokLongContextFollowsGroupToggleOnly(t *testing.T) {
	...
	t.Run("group on applies the official ladder", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.True(t, usageRepo.lastLog.LongContextBillingApplied)
		require.InDelta(t, baseInput*2*3.5, usageRepo.lastLog.InputCost, 1e-10)
		require.InDelta(t, baseOutput*2*3.5, usageRepo.lastLog.OutputCost, 1e-10)
	})
	...
	t.Run("group off keeps the base card", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.False(t, usageRepo.lastLog.LongContextBillingApplied)
		require.InDelta(t, baseInput*3.5, usageRepo.lastLog.InputCost, 1e-10)
		require.InDelta(t, baseOutput*3.5, usageRepo.lastLog.OutputCost, 1e-10)
	})
}
