...
INSERT INTO users VALUES (1,100.25,NULL),(2,25.50,NULL),(3,-999,NULL),(4,0,NULL),(5,50000,now());`)
	...
	_, err = tx.ExecContext(ctx, `INSERT INTO usage_logs (user_id, created_at, actual_cost) VALUES
	 ...
	 (2,$2,4.00),(2,$1::timestamptz - interval '1 microsecond',8.00),(3,$1,3.00),(4,$1,4.00),(5,$1,100.00)`, start, end)
	require.NoError(t, err)
	repo := newUserRepositoryWithSQL(nil, tx)
	stats, ids, err := repo.GetAdminUserOverview(ctx, start, end, start.Add(-24*time.Hour))
	require.NoError(t, err)
	...
	require.InDelta(t, 18.75, stats.TodayUserCost, 0.000001)
