...
CREATE TEMP TABLE ops_error_logs (
 ...
 status_code integer,upstream_status_code integer,upstream_error_message text,upstream_error_detail text,upstream_errors jsonb,api_key_id bigint
) ON COMMIT DROP;
CREATE TEMP TABLE usage_logs(id bigint,request_id text,account_id bigint,created_at timestamptz,output_tokens integer,image_count integer,first_token_ms integer,api_key_id bigint) ON COMMIT DROP;
...
INSERT INTO usage_logs VALUES (9,'probe',1,'2026-09-16T03:59:00Z',20,0,99999,99);
