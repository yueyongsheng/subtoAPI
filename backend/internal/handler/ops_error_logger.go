...
func observeChannelHealth(c *gin.Context, observer ChannelHealthObserver, startedAt time.Time) {
	...
	observer.ObserveChannelHealth(service.ChannelHealthObservation{
		GroupID: *apiKey.GroupID, APIKeyID: apiKey.ID, Model: model,
		Success: success, RecoveredError: success && hasUpstreamFailure,
		QualifyingFailure: qualifyingFailure, Duration: duration, ObservedAt: time.Now(),
	})
}
