...
// Saving settings is a whole-document PUT. Automation must read the current
// document, change its target field, then submit the complete document.
func TestUpdateSettingsSparsePayloadUsesWholeDocumentSemantics(t *testing.T) {
	...
	require.Equal(t, "", repo.values[service.SettingKeySiteSubtitle])
	// Sensitive values keep their established empty-means-preserve behavior.
