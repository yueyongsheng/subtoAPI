...
func TestMigration180ConvertsLegacyRechargeBalancesAndPricingExactlyOnce(t *testing.T) {
	...
	require.Contains(t, sql, "1to1_balance_cutover_v1")
	...
	require.Contains(t, sql, "('default_balance', '0.10000000'")
	require.Contains(t, sql, "('BALANCE_RECHARGE_MULTIPLIER', '1.00000000'")
	require.Contains(t, sql, "rate_multiplier = 1.4000")
	...
	require.Contains(t, sql, `"gpt-5.5"`)
	require.Contains(t, sql, `"gpt-5.4"`)
	require.Contains(t, sql, `"codex-auto-review"`)
	require.Contains(t, sql, "id IN (22, 23, 24, 25)")
	require.Contains(t, sql, "id = 13")
	require.Contains(t, sql, "id = 14")
	require.Contains(t, sql, "id = 26")
