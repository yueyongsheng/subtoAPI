export default {
    accounts: {
      ...
      oauthHealth: {
        ...
        rules: '系统建议范围 5～30，恢复每次 +5。持续 429/503 达到至少 5 个请求、3 个分钟、20% 占比且近 5 分钟仍有异常时降载；最低 5，仍异常则建议冷却 3 分钟。每次调整后观察 30 分钟；增加后异常可回退并观察 60 分钟。恢复需有正常样本及实际负载，手动并发优先。检测只生成建议，点击后执行。',
        cooldownAction: '冷却 3 分钟',
        pressureSummary: '429/503 压力请求 {count} / {total}（{rate}%），最近一次：{last}（UTC+8）',
        shortEvidence: '压力请求 {count} / {total}',
        currentLoad: '检测时实际并发 {value} / {limit}',
        recoveryModels: '恢复验证模型：{models}',
        holdUntil: '观察期至 {time}（UTC+8）',
        cooldownUntil: '冷却至 {time}（UTC+8）',
        checkedAt: '检测时间', historical: '历史检测 · ', checking: '正在分析近期请求…',
        summary: '检测 {total} 个 OAuth 账号，{recommended} 个处理建议',
        ...
        recheck: '重新检测', apply: '应用推荐处理（{count}）', restore: '恢复至 {value}',
        applied: '已处理 {done} 个账号，跳过 {skipped} 个。账号状态或检测结果变化时会跳过，请重新检测后查看。',
        failed: '检测或处理未完成。已保存的结果会保留，请重新检测后再操作。',
        status: { insufficient: '待观察', stable: '近期稳定', recovered: '已恢复', rate_limited: '出现限流', quota_limited: '额度受限', overloaded: '上游 503', auth_error: '授权异常', upstream_error: '上游异常' },
        reason: {
          manual_concurrency: '手动并发优先，仅检测并保留配置。',
          model_observation: '先前异常模型的正常样本不足，继续观察。',
          rollback_increase: '增加后再次持续异常，建议退回上一个并发值，观察 60 分钟。',
          cooldown_recommended: '已到最低并发，仍持续异常；建议账号临时冷却 3 分钟。共用分组已有近期成功的同模型备用账号，冷却结束后原并发重新参与调度。',
          no_cooldown_alternative: '已到最低并发，但部分共用分组缺少近期成功的同模型备用账号；保留配置，先补充可用线路。',
          maximum_concurrency: '近期稳定，已达到系统建议上限 30；更高并发请手动设置。',
          recovery_observation: '开始积累恢复证据，至少观察 30 分钟后再建议增加。',
          recovery_samples: '正常请求尚未覆盖至少 3 个分钟，继续观察。',
          load_unavailable: '当前并发统计暂不可用，本次不建议增加。',
          low_demand: '近期已稳定，当前实际并发未达上限的 80%，暂不需要增加。',
          increase_concurrency: '观察期已结束，相关模型样本正常且存在负载需求；建议 +5，最高 30。旧版系统降低到 5 以下的账号先恢复至 5。',
          ...
          cooldown: '账号仍在临时冷却中，保持现有设置。', change_cooldown: '调整后观察新请求，到观察期结束再评估；增加并发后持续异常可提前回退。',
