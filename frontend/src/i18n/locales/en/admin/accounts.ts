export default {
    accounts: {
      ...
      oauthAvailability: {
        ...
        allCount: 'All {count}',
        viewAll: 'View all {count} groups',
        allGroups: 'All groups',
        groupName: 'Group name',
        availableAccounts: 'Available OAuth',
        sortAscending: 'Fewest available accounts first; ties ordered by group ID',
      ...
      oauthHealth: {
        ...
        rules: 'Recommendations stay within 5–30, increasing by 5. Reduce for at least 5 pressure requests across 3 minutes, a 20% share and an error within 5 minutes; at the floor, recommend a 3-minute cooldown. Observe changes for 30 minutes; failed increases can roll back with a 60-minute hold. Recovery needs successful samples and load. Manual settings take priority. Apply recommendations explicitly.',
        cooldownAction: 'Cool for 3 minutes',
        pressureSummary: '429/503 pressure: {count} / {total} ({rate}%). Latest: {last} (UTC+8)',
        shortEvidence: 'Pressure requests {count} / {total}',
        currentLoad: 'Concurrency at check: {value} / {limit}',
        recoveryModels: 'Models requiring recovery evidence: {models}',
        holdUntil: 'Observe until {time} (UTC+8)',
        cooldownUntil: 'Cooldown until {time} (UTC+8)',
        checkedAt: 'Checked at', historical: 'Past check · ', checking: 'Analyzing recent requests…',
        summary: '{total} OAuth accounts checked; {recommended} actions recommended',
        ...
        status: { insufficient: 'Needs observation', stable: 'Recently stable', recovered: 'Recovered', rate_limited: 'Rate limits observed', quota_limited: 'Quota limited', overloaded: 'Upstream 503', auth_error: 'Authorization error', upstream_error: 'Upstream error' },
        reason: {
          manual_concurrency: 'Manual concurrency takes priority. Report only.',
          model_observation: 'Previously affected models need more successful samples.',
          rollback_increase: 'Sustained errors returned after an increase. Roll back and observe for 60 minutes.',
          cooldown_recommended: 'At the floor with sustained errors. Cool this account for 3 minutes; each group has recent successful alternatives for the affected models. Resume at the same limit after cooldown.',
          no_cooldown_alternative: 'At the floor; a shared group lacks a recently successful alternative for an affected model. Keep settings and add capacity.',
          maximum_concurrency: 'Recently stable and at the recommendation ceiling of 30. Higher limits require a manual edit.',
          recovery_observation: 'Collect recovery evidence for at least 30 minutes before increasing.',
          recovery_samples: 'Successful requests need to span at least 3 minute buckets.',
          load_unavailable: 'Current concurrency is unavailable. No increase recommended.',
          low_demand: 'Recently stable; current concurrency is below 80% of the limit. No increase needed.',
          increase_concurrency: 'Observation passed with successful model samples and demand. Increase by 5, up to 30; legacy reductions below 5 first return to 5.',
          ...
          cooldown: 'Temporary cooldown remains active. Keep the current settings.', change_cooldown: 'Observe new requests until the hold ends; a failed increase can roll back early.',
