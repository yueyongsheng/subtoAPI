    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="flex flex-wrap items-center gap-x-1 text-xs text-gray-500">
          ...
          <span>/</span>
          <span class="group relative inline-flex max-w-full flex-wrap cursor-help items-center gap-0.5" tabindex="0">
            <span>{{ cacheLabel() }}: {{ formatTokens(stats?.total_cache_tokens || 0) }}</span>
            <span v-if="showCacheHitRate" class="tabular-nums" :title="t('usage.cacheHitRateHint')">
              ({{ t('usage.cacheRate') }}: {{ cacheHitRate }})
            </span>
            <svg
            ...
            >
              ...
              <span v-if="showCacheHitRate" class="mt-2 block border-t border-gray-200 pt-2 dark:border-dark-600">
...
const cacheHitRate = computed(() => {
  // 使用当前筛选范围的原始 Token 汇总，缓存创建只计入分母，输出不参与。
  const cacheRead = Math.max(0, props.stats?.total_cache_read_tokens || 0)
  const totalInput = Math.max(0, props.stats?.total_input_tokens || 0)
    + Math.max(0, props.stats?.total_cache_creation_tokens || 0) + cacheRead
