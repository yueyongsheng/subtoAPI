...
goals = true`
  ...
  if (codexAuthMode.value === 'api-key') {
    const envFile = isWindows
      ? { path: 'PowerShell', content: `$env:OPENAI_API_KEY="${apiKey}"` }
      : { path: 'Terminal', content: `export OPENAI_API_KEY="${apiKey}"` }
    return [
      {
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  // Codex config reference: wire_api = "responses" only; credentials come from env_key.
...
name = "Sub2API Grok"
base_url = "${baseUrl}"
# Credentials come from the environment variable; avoid writing API keys to config files.
