...
const exportToExcel = async () => {
  ...
  try {
    ...
    const spreadsheetRows: Array<Array<{ value: string | number }>> = [
    ...
    while (true) {
      ...
      if (rows.length) {
        spreadsheetRows.push(...rows.map(row => row.map(value => ({ value: value ?? '' }))))
      }
    ...
    if(!c.signal.aborted) {
      await writeXlsxFile(spreadsheetRows, { sheet: 'Usage' })
...
const HIDDEN_COLUMNS_KEY = 'admin-usage-hidden-columns'
...
const loadSavedColumns = () => {
  try {
    // Read the pre-v0.2.1 key once, then persist under the admin-scoped key.
    let saved = localStorage.getItem(HIDDEN_COLUMNS_KEY)
    if (!saved) {
      saved = localStorage.getItem(LEGACY_HIDDEN_COLUMNS_KEY)
      if (saved) localStorage.setItem(HIDDEN_COLUMNS_KEY, saved)
    }
    if (saved) {
