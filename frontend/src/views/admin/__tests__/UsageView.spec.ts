...
const { list, exportList, getStats, getSnapshotV2, getById, getModelStats, listErrorLogs, routeQuery, writeXlsxFile, writeXlsxFileToFile } = vi.hoisted(() => {
  ...
  const writeXlsxFileToFile = vi.fn().mockResolvedValue(undefined)
  ...
  return {
    ...
    routeQuery: {} as Record<string, string>,
		writeXlsxFile: vi.fn(() => ({ toFile: writeXlsxFileToFile })),
		writeXlsxFileToFile,
  }
})
...
describe('admin UsageView model audit export', () => {
	...
	it('exports requested, sent, response, and mismatch as separate admin columns', async () => {
		...
		const spreadsheetRows = writeXlsxFile.mock.calls[0][0]
		const headers = spreadsheetRows[0].map((cell: { value: string }) => cell.value)
		...
		const row = spreadsheetRows[1].map((cell: { value: string | number }) => cell.value)
		expect(row.slice(4, 8)).toEqual(['gpt-5.6-sol', 'gpt-5.5', 'gpt-5.4', 'Yes'])
		expect(writeXlsxFile).toHaveBeenCalledWith(spreadsheetRows, { sheet: 'Usage' })
		expect(writeXlsxFileToFile).toHaveBeenCalledWith(expect.stringMatching(/^usage_.+\.xlsx$/))
	})
})
