...
describe('HomeView compact mode', () => {
  ...
  it('uses the model plaza as the compact home destination for unauthenticated visitors', () => {
    expect(compactDestination(mountHome({ compact_home_enabled: true }))).toBe('/model-plaza')
  })

  it('keeps the model plaza as the compact home destination for authenticated users', () => {
    authStore.isAuthenticated = true

    expect(compactDestination(mountHome({ compact_home_enabled: true }))).toBe('/model-plaza')
  })

  it('keeps the model plaza as the compact home destination for administrators', () => {
    authStore.isAuthenticated = true
    authStore.isAdmin = true

    const wrapper = mountHome({ compact_home_enabled: true })
    expect(compactDestination(wrapper)).toBe('/model-plaza')
